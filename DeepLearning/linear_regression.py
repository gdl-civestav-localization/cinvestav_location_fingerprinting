...
# Load training data.
train_x = np.asarray([[0.0], [1.0], [2.0], [3.0], [4.0]], dtype=theano.config.floatX)
train_y = np.asarray([1.0, 1.1, 3.0, 2.0, 5.0], dtype=theano.config.floatX)
...
w = theano.shared(
    value=np.zeros(
        (n_dims, 1),  # Parameters of the model.
